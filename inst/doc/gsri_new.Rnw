...
In this introduction we will use a sample data for expression data
provided by the \Rpackage{Biobase} package. The data set contains
26 microarrays with a subset of 500 probe sets. The phenotypes associated
with the samples are stored in the \Robject{ExpressionSet}, including
the categorial variables \emph{sex} and \emph{type} of disease, as
well as the continous \emph{score} indicating the progress of the
disease.
...
An differential effect of features has to be defined in respect to
a categorial variable, in our case we will use the phenotypes.
