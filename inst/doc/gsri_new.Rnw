...
\author{Julian Gehring, Clemens Kreutz, Kilian Bartholomé, Jens Timmer}
...
with the fraction $r$ of significant $p$, the uniform distribution
...
CDF with slope $1-r$. Through iterative fitting of this linear component,
$r$ and thus the number of regulated genes can be estimated. An example
for the probability and cumulative density distribution is shown in
Figure \ref{fig:pval_cdf_dist}.

\begin{figure}
\centering
\includegraphics[width=0.5\columnwidth]{x_gsri_pdf}\includegraphics[width=0.5\columnwidth]{x_gsri_cdf}
...
\Rpackage{Biobase} package. It contains the expression intensities
...
including the categorical variables \Rvar{sex} and \Rvar{type} of
disease represented as factors, as well as the contiguous \Rvar{score}
...
effect between the groups \Rvar{sex} and \Rvar{type}. In a first
...
However, taking the corresponding standard deviation of around 15\%
and the GSRI of 0 at the 5\% level into account, we can doubt to look
at differential effect.

In the next step we exclude the controls of the Affymetrix microarray
since they are not relevant for our analysis. For this we define an
object \Robject{gsAllGenes} of the class \Rclass{GeneSet} with the
subset of genes of interest. Note that in this case we could also
use a subset of \Rvar{eset} or \Rvar{exprs} without an additional
...
the progress of the disease. For this analysis we use the F-test provided
by this package.
...
The GSRI approach has several parameters that can be changed for the
analysis. For illustration we rename the gene set, change the number
of bootstraps and confidence level for the GSRI calculation, and use
a classical ECDF instead of the modified Grenander estimator for the
cumulative density.
...
\Rpackage{limma} package which as an enhanced power for small sample
...
$\rho_{0}(p)$ as well as the estimated fraction $r$ of significant
genes and the GSRI.
...
the help pages for the \Rmethod{plot} method.
...
does the GSRI package also allow assign weights for each gene in the
calculation. Such a step is useful for including additional information
in the estimation process, for example the certainty that a gene is
part of a gene set.
