...
function (data, d, phenotype, test, testArgs) 
{
    ...
    pvals <- multiStat(as.matrix(data), phenotype, test, testArgs)
    ...
    test = "ttest", testArgs = NULL, alpha = 0.05, prec = 4) 
{
    ...
    pt <- b$t - bias
    gsri <- quantile(pt, alpha, na.rm=TRUE)
    d <- 1:nSamples
    pvals <- getPvalues(data, d, phenotype, test, testArgs)
    nGenes <- length(pvals)
    ...
    alpha = 0.05, verbose = TRUE, prec = 4) 
{
    ...
    data <- loadGct(dataFileName)
    if (verbose == TRUE) 
        message(sprintf("%s '%s' %s", "Data file", basename(dataFileName), 
            "loaded"))
    phenotype <- loadCls(phenotypeFileName)
    if (verbose == TRUE) 
        message(sprintf("%s '%s' %s", "Phenotype file", basename(phenotypeFileName), 
            "loaded"))
    ...
    for (i in 1:length(geneSets)) {
        ...
        geneSetName <- temp[1]
        geneSet <- list()
        geneSet$genes <- temp[3:length(temp)]
        geneSet$data <- data[intersect(row.names(data), geneSet$genes), 
            ]
        geneSet$genes <- row.names(geneSet$data)
        geneSet$genesResults <- paste(row.names(geneSet$data), 
            collapse = " : ")
        geneSetSize <- dim(geneSet$data)[1]
        if (geneSetSize >= minGeneSetSize) {
            if (verbose == TRUE) 
                message(sprintf("%s '%s' (%d %s)", "Calculate GSRI for", 
                  geneSetName, geneSetSize, "genes"))
            ...
            res$numRegGenesSd[c] <- signif(gsriRes$numRegGenesSd, 
                digits = prec)
            res$gsri[c] <- signif(gsriRes$gsri, digits = prec)
            res$nGenes[c] <- signif(gsriRes$nGenes, digits = prec)
            res$geneSetGenes[c] <- geneSet$genesResults
            c <- c + 1
        }
    }
    if (writeSummary == TRUE) {
        resultFile <- file.path(getwd(), paste(strsplit(dataFileName, 
            ".gct")[[1]], "_results.txt", sep = ""))
...
{
    if (!is.function(test) && test == "ttest") 
        pvals <- rowttests(data, phenotype)$p.value
    if (!is.function(test) && test == "ftest") 
        pvals <- rowFtests(data, phenotype)$p.value
    if (is.function(test) && is.null(testArgs)) 
        pvals <- test(data, phenotype)
    if (is.function(test) && !is.null(testArgs)) 
        pvals <- test(data, phenotype, testArgs)
    return(pvals)
}
slopeFast <-
function (x, y) 
